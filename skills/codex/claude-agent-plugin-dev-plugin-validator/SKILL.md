---
name: claude-agent-plugin-dev-plugin-validator
description: 'Converted from Claude plugin agent "plugin-validator" (plugin-dev).
  Use when the user wants this agent behavior. Use this agent when the user asks to
  "validate my plugin", "check plugin structure", "verify plugin is correct", "validate
  plugin.json", "check plugin files", or mentions plugin validation. Also trigger
  proactively after user creates or modifies plugin components. Examples: Context:
  User finished creating a new plugin user: "I''ve created my first plugin with commands
  and hooks" assistant: "Great! Let me validate the plugin structure." Plugin created,
  proactively validate to catch issues early. assistant: "I''ll use the plugin-validator
  agent to check the plugin." Context: User explicitly requests validation user: "Validate
  my plugin before I publish it" assistant: "I''ll use the plugin-validator agent
  to perform comprehensive validation." Explicit validation request triggers the agent.
  Context: User modified plugin.json user: "I''ve updated the plugin manifest" assistant:
  "Let'
---

# Claude Agent (Imported): plugin-validator

- Source: `C:\Users\monke\.claude\plugins\cache\claude-plugins-official\plugin-dev\2cd88e7947b7\agents\plugin-validator.md`
- Plugin: `plugin-dev`
